...
\pmcfg{}, including context-free grammars (\cfg), formalisms such as Tree-Adjoining Grammar (\tagGrammar)
...
The set of most unique argument trees for
some $f \in F^C$ is computed by the function
$\text{\tt bestTrees}:~F^C~\rightarrow~ [[\t{Tree}]]$.  We can access
the argument categories $f$ by calling \t{args} :
$F^C \rightarrow [N^C]$.

For an $f$ with arguments $[A_1,A_2,\dots,A_{a(f)}]$, the Trees in the
inner lists are of types $[A_1,A_2,\dots,A_{a(f)}]$. Instead of just
one list of arguments, the function
returns a list of lists: as explained before, sometimes multiple
arguments are needed (like \emph{you} and \emph{she}) in order to ensure different strings in all fields of the argument tree.
...
bestTrees f =
  let argCats = args f
   in mostUniqueExamples f (generateAllTrees argCats)
\end{HighlightingFancy}
\end{EmptyItem}

The function \t{generateAllTrees :} $[N^C] \rightarrow [[$\t{Tree}$]]$  is implemented using the \feat{} library \cite{feat}, and generates lazily all vectors of argument trees for the given concrete categories, starting from smallest size. 
...
Before we get to \t{mostUniqueExamples}, we define two helper functions: \t{betterTest} and \t{allUnique}. \t{betterTest} decides among two lists of strings, which one of them is a better test: e.g. [{\em I},{\em me},{\em my}] is better than [{\em you},{\em you},{\em your}]. \t{allUnique} takes a list of such lists, and decides if it is enough: e.g. [[{\em I},{\em me},{\em my}]] fulfils the condition, as well as [[{\em you},{\em you},{\em your}], [{\em she},{\em her},{\em her}]].

\todo{Improve the description of algorithms, as Koen suggested}

\begin{EmptyItem}
\begin{HighlightingFancy}[]
betterTest : [String] $\rightarrow$ [String] $\rightarrow$ [String]
betterTest xs ys =
...
allUnique : [[String]] $\rightarrow$ Bool
allUnique $[[a_{1,1},a_{1,2}, \dots, a_{1,m}], \ldots, [a_{n,1},a_{n,2}, \dots, a_{n,m}]]$ =
 \CommentTok{if} $\exists \{ a_{j_i,i} | i \in \{1..m\}, j_i \in \{1..n\} \}$ \CommentTok{which has} $m$ \CommentTok{unique elements}.
\end{HighlightingFancy}
\end{EmptyItem}

Now we define \t{mostUniqueExamples} $: F^C \rightarrow [[$\t{Tree}$]] \rightarrow [[$\t{Tree}$]]$, which returns a minimal set of arguments that is needed to test the function $f$.
...
\begin{figure}[h]
\includegraphics[width=\textwidth]{img/kind-contexts-with-rabbits.png}
\caption{\todo{Make better graphics} Left: Context for \t{A} in start
  category \t{S} depends on a context for \t{B}. Right: Two contexts for the
  category \t{CN}, which choose all forms from the inflection table.}
...
shows an example of a context in general, as a tree with a hole of
type \t{A}, and a concrete example for the category \t{CN} in our
example grammar.
...
form.  This suggests that we should apply two different \t{CN
  $\rightarrow$ NP} functions, for instance \t{DetCN this} and
\t{DetCN these}, and give their results to the \t{UttNP} function,
which constructs an \t{S}.
...
Figure~\ref{tab:adjAttr} into the holes, and get 8 trees in total as
shown in Figure~\ref{tab:testCases}.
...
up into an \t{NP}, just like in Figure~\ref{fig:context-general}, and
only then can \t{UttNP} get access to its strings.

We model this in a top-down manner, where in the beginning, only
...
unlocked ones, until all categories have their
contexts. Figure~\ref{fig:context-general} shows the general
pattern for categories \t{A} and \t{B}.
...
We want to find a minimal collection of sets of contexts (one set for
each category), such that a given set of equations if fulfilled. 
