...
We use one syntactic function $f \in F^C$ as the base for one set of test
...
\todo{something more formal here?}
...
The majority of all functions in all tested languages need 1 or
...
of all lists of argument trees. First we define the function
\todo{better name for $\textsf{f}$} that the fold takes as its first
...
\begin{EmptyItem}
\begin{array}{p{0.25cm} p{0.2cm} l}
\multicolumn{3}{l}{\textsf{f :: } F^C  \rightarrow \textsf{[Tree]} \rightarrow \textsf{[[Tree]]} \rightarrow \textsf{[[Tree]]}} \\
\multicolumn{3}{l}{\textsf{f } f \textsf{ t ts } = \textbf{if } \textsf{any } (\lambda x. \textsf{ subsumes } f \; x \textsf{ t) ts}} \\
...
\textsf{mostUniqueExamples } f \textsf{ vtrees} = \textsf{foldr (f } f \textsf{) [] vtrees} \\
\end{array}
\end{EmptyItem}
...
\begin{EmptyItem}
\begin{array}{p{0.25cm} p{0.2cm} p{0.8cm} l}
...
\paragraph{Top-down computation}
...
using fixpoint iteration. \todo{would be inefficient to do it bottom-up.}
...
\noindent In the general case, these sets can be infinite: think of
functions of type \t{A~$\rightarrow$~A} and loops of \t{A~$\rightarrow$~B} and \t{B~$\rightarrow$~A}.  We want to find the
{\em least} solution to such systems of equations. 

The least set,
i.e. no junk--we don't want infinite sets. We are only interested in
those contexts that cover all.

 The word \emph{least} means, in
...
It is possible to express the set of relevant contexts for one hole type $H$ in terms of the sets of relevant contexts for other hole types $H'$:
...
in $H'$. \todo{Kleene's or Tarski's fixpoint theorem + read Koen's slides}
