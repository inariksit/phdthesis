...
$f \in F^C$, we need to supply the function with \emph{arguments}, as
well as put the resulting tree into a \emph{context} that produces a
tree in the correct start category.  In the best case, each $f$
produces only one tree, but as we will explain in the following
section, sometimes we need more than one tree for a single concrete
function; this happens if there are no arguments with unique string
fields.
...
\multicolumn{3}{l}{\textsf{linAll :: Tree } \rightarrow \textsf{ [String]}} \\
\multicolumn{3}{l}{\textsf{linAll t} = \textbf{return all string
  fields of }\textsf{t}} \\
\\
...
\multicolumn{3}{l}{\textsf{subsumes } f \textsf{ xtrees ytrees} =} \\
 & \textbf{let} & \textsf{syncats = } \textbf{strings introduced by } f \\
 &              & \textsf{xlins} = \textsf{syncats} \text{ ++ } \textsf{concatMap linAll
                  xtrees} \\
 &              & \textsf{ylins} =  \textsf{syncats} \text{ ++ } \textsf{concatMap linAll
                  ytrees} \\
 &              & \textsf{len = min xs ys}
 & \textbf{in}  & \textsf{and [ ylins !! } i  \text{ ==} \textsf{ ylins !! } j \\
 &              & \textsf{| } i \leftarrow \textsf{[0..len-1], } j \leftarrow \textsf{[0..len-1]} \\
 &              & \textsf{, xs !! } i == \textsf{ xs !! } j \textsf{ ]} \\
% \forall i,j : \textbf{ if } \textsf{xlins } ! \, i =
%    \textsf{xlins } ! \, j,
...
element of $\textsf{xs}$ ($x_i$) is the same as the $j^{th}$ element
($x_j$), then also the corresponding elements from $\textsf{ys}$
($y_i$ and $y_j$) must be the same. If this property holds, then
$\textsf{xs}$ subsumes $\textsf{ys}$.
...
\multicolumn{3}{l}{\textsf{generateAllTrees
  [a}_1,\ldots,\textsf{a}_n\textsf{] = } \textbf{generates lazily all vectors of
argument trees for the given concrete categories, starting from
smallest size. (defined in FEAT)}}
\end{array}
\end{EmptyItem}
\end{figure}

The main logic, in $\textsf{mostUniqueExamples}$, is implemented as a
fold over the lazily generated list of all lists of argument
trees. First we define the function $\textsf{keepIfRelevant}$ that the
fold takes as its first argument: it takes a new tentative test case
...
\begin{figure}[h]
\begin{EmptyItem}
\begin{array}{p{3.45cm} l}
\multicolumn{2}{l}{\textsf{keepIfRelevant :: } F^C  \rightarrow \textsf{[Tree]} \rightarrow \textsf{[[Tree]]} \rightarrow \textsf{[[Tree]]}} \\
\multicolumn{2}{l}{\textsf{keepIfRelevant } f \textsf{ t ts } = \textbf{if } \textsf{any } (\lambda x. \textsf{ subsumes } f \; x \textsf{ t) ts}} \\
  & \textbf{then} \textsf{ ts} \\
...
$f$. The argument \todo{vtrees is the list of all trees, given by
FEAT.generateAll. The algorithm starts from an empty list, and adds a
new item whenever it finds one that doesn't subsume. When it adds a
new item, it checks whether any of the old ones is subsumed by it, and
removes redundant stuff.}

%, as a fold over all examples. 

In practice, we stop after examining 
10,000 lists of trees, or whenever we have found a desired combination 
of trees.
...
\multicolumn{4}{l}{\textsf{bestTrees} \; f =} \\
 ...
 &    & &                             | \;A \rightarrow
          f[A_1,A_2,\dots,A_{a(f)}] is a production in P. \; \} \\
...
under test (denoted by \char`_). \todo{hole is a function}
...
if one wants to test the whole grammar at one go. 
\todo{Drawback: how to interpret the test results: a bug can come from
...
why'' \todo{real example} may appear in the generated test cases. 
