...
human-written treebank can be found in \citet[p.~136--142]{khegai2006phd}.
...
for parsing grammars, such as 
\citet{gardent2012errormining}. The setup includes triples of dependency
...
the error.
Further away from our approach, \citet{vanNoord2004}
...
\citet{hamon2004testgen}. Software testing and grammar testing both
...
separately. Each concrete syntactic function may produce one or several trees.
...
by the function $\textsf{bestTrees}$. %~:~}F^C~\rightarrow~[[\textsf{Tree}]]$.
...
%For instance, if $[N^C]$ is $[$\t{Prep},\t{NP}$]$,
...
Before we get to \textsf{mostUniqueExamples}, we define two helper functions: \textsf{betterTest} and \textsf{allUnique}. \textsf{betterTest} decides among two lists which one of them is a better test: e.g. [{\em I},{\em me},{\em my}] is better than [{\em you},{\em you},{\em your}]. \textsf{allUnique} takes a list of such lists, and decides if it is enough: e.g. [[{\em I},{\em me},{\em my}]] fulfils the condition, as well as [[{\em you},{\em you},{\em your}], [{\em she},{\em her},{\em her}]].
  ...
  % let (x$_{\text{1}}$,y$_{\text{1}}$):xs-ys = zip xs ys
  % in  $\forall$ (x$_{\text{i}}$,y$_{\text{i}}$) $\in$ xs-ys where x$_{\text{1}}$ $=$ x$_{\text{i}}$:
  %       if $\exists$ y$_{\text{i}}$ such that y$_{\text{1}}$ $\neq$ y$_{\text{i}}$,
  %        then \CommentTok{ys are better tests}.
  %        else \CommentTok{repeat for} \{ (x$_{\text{j}}$,y$_{\text{j}}$) | (x$_{\text{j}}$,y$_{\text{j}}$) $\leftarrow$ xs-ys, x$_{\text{j}}$ $\neq$ x$_{\text{1}}$ \}
  %     if xs-ys \CommentTok{is empty},
  %      then \CommentTok{xs are better tests}.

\begin{EmptyItem}
\begin{array}{p{0.25cm} l}
\multicolumn{2}{l}{\textsf{betterTest : }[a] \rightarrow [a] \rightarrow [a]} \\
\multicolumn{2}{l}{\textsf{betterTest xs ys} =} \\
 & \textsf{xs} \textbf{ is a better test than } \textsf{ys}, \textbf{ if whenever } x_i
...
Now we define $\textsf{mostUniqueExamples}$, which returns a minimal set of arguments that is needed to test the function $f$.

\begin{EmptyItem}
\begin{array}{p{0.25cm} p{0.2cm} p{1.4cm} l}
\multicolumn{4}{l}{\textsf{mostUniqueExamples : } F^C \rightarrow
  [[\textsf{Tree}]] \rightarrow [[\textsf{Tree}]]} \\
\multicolumn{4}{l}{\textsf{mostUniqueExamples } f \textsf{ vtrees} =} \\
   & \textbf{let} & \multicolumn{2}{l}{\textsf{trees-lins} = \{ \; (\textsf{vtree}\textbf{, strings of}
    \textsf{ vtree}\text{ ++ }\textbf{strings that } f \textbf{ introduces})} \\
  &     &        & | \textsf{ vtree} \in \textsf{vtrees} \} \\
  & \textbf{in}  & \multicolumn{2}{l}{\textbf{apply}  \textsf{ betterTest } \textbf{to}
                   \textsf{ lins } \textbf{of} \textsf{ trees-lins } \textbf{until}  \textsf{ allUnique}} \\
  & & \multicolumn{2}{l}{\textbf{return} \textsf{ trees } \textbf{of} \textsf{
      trees-lins}}  \\
\end{array}
\end{EmptyItem}
...
We also stop (give up) when too many tries have been made (currently
...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{img/contexts-example.png}
...
nothing, you're already there!''. On the first round, we get to
compute contexts for all categories \t{B} that are arguments to some
function \t{g : B $\rightarrow$ S}: these contexts are simply the
function \t{g} itself. On the second round, we compute contexts for
all categories \t{A} that are arguments to some function \t{f : A
  $\rightarrow$ B}; the context for such an \t{A} is \t{g~$\circ$~f}. Each
round unlocks more categories further down from the start category, so
we get to compute more contexts for categories that depend on the
newly unlocked ones, until all categories have their
contexts. Figure~\ref{fig:context-dependence} shows the general
pattern for categories \t{A} and \t{B} in start category \t{S}.
is
...
system of equations. For instance, the simple example with
\t{f~:~A~$\rightarrow$~B} and \t{g~:~B~$\rightarrow$~S} is expressed
as follows.
...
% for \t{PrepNP}. 
% Table~\ref{info-on-grammar} is defined as follows for every concrete category \t{C} in the \pmcfg{}.

% $$
% C \mapsto \{ f, i, \text{\it fields}(C), R \;|\; \forall f \in F^C, f : C_1 \rightarrow \dots
%   \rightarrow C_i \rightarrow C_n \rightarrow R \}
% $$

Computing relevant contexts in a given start category \t{S} is done once, in advance, for all possible hole types $H$ at the same time, using fixpoint iteration. It is possible to express the set of relevant contexts for one hole type $H$ in terms of the sets of relevant contexts for other hole types $H'$:
...
we use the following definition $\textsf{contexts}(\t{S}) = \{ \; id
\; \}$. In order to compute all sets of contexts for all possible hole
categories $H$, we set up a system of equations (as specified
above). In general, this system of equations is recursive, and we use
a fixpoint iteration to solve it, starting with
$\textsf{contexts}(H)~=~\varnothing$ for each category $H$ that is
not the start category.
% the empty set $\varnothing$ for each set.
There is a guaranteed minimal solution, because the RHSs are monotonic
in $H'$. \todo{Do I need to cite something? Expand on this?}
